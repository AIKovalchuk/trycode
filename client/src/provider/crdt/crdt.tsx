...
const CRDT: React.FC<Props> = ({ session, children }) => {
    ...
    const compareChars = (charA: Char, charB: Char) => {
        ...
        for (let deep = 0; deep < length; deep++) {
            ...
            } else if (charA.position[deep] === charB.position[deep] && deep === length - 1) {
                return 0;
            }
        }
        return 0;
    ...
    const findPosBetween = (posBefore: number[], posAfter: number[], newPos: number[] = []): number[] => {
    ...
    const findPosBefore = (pos: EditorPosition) => {
        ...
        if (ch === 0 && line === 0) {
            ...
            const posBef = charsRef.current[line - 1][lengthOfLineBefore - 1].position;
            return posBef;
        } else {
    ...
    const findPosAfter = (pos: EditorPosition) => {
        ...
        const lengthOfLine = (charsRef.current[line] && charsRef.current[line].length) || 0;
    ...
    const findPositionChar = (char: Char) => {
        ...
        if (charsRef.current[line] && charsRef.current[line][ch - 1] && charsRef.current[line][ch - 1].char === '\n') {
            line += 1;
            ch = 0;
        }
    ...
    const findLine = (char: Char) => {
        ...
        if (
            compareChars(
                char,
                charsRef.current[charsRef.current.length - 1][charsRef.current[charsRef.current.length - 1].length - 1],
            ) === 1
        ) {
    ...
    const deleteChar = (posStart: EditorPosition, posEnd: EditorPosition) => {
        ...
        if (posEnd.line !== posStart.line) {
            ...
            charsRef.current[posStart.line] = charsRef.current[posStart.line].concat(chars);
    ...
    const handleLocalDelete = (posStart: EditorPosition, posEnd: EditorPosition) => {
    ...
    const handleRemoteDelete = React.useCallback((char: Char) => {
        ...
        if (charsRef.current[pos.line + 1] && charsRef.current[pos.line].length - 1 === pos.ch) {
    ...
    const getText = () => {
        return charsRef.current.map((line) => line.map((ch) => ch.char).join('')).join('');
    };
